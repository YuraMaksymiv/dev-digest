/* FindingsPanel — per-severity counters + hide-low-confidence + j/k navigation
...
export function FindingsPanel({
  ...
  // Counts always come from the FULL list, so they stay put while filtering.
        ...
        {findings.length > 0 && (
          <div style={s.counterGroup} role="group" aria-label={t("panel.severityCounters")}>
            {counts.map(([sev, count]) => {
              // The badge's own text ("Critical 2") would otherwise be the
              // accessible name — say what the click DOES instead.
              const label =
                severityFilter === sev

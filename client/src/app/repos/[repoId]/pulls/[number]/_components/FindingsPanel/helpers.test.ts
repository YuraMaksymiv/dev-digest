...
describe("severityCounts", () => {
  it("counts per severity in display order", () => {
  ...
  it("counts dismissed findings too — the panel still renders their cards", () => {
    ...
    expect(severityCounts([dismissed])[0]).toEqual(["CRITICAL", 1]);
  });
});
...
describe("visibleFindings severity filter", () => {
  ...
  it("no filter keeps everything, sorted by severity", () => {
    expect(visibleFindings(FINDINGS, false, null).map((f) => f.severity)).toEqual([
      "CRITICAL",
      "CRITICAL",
      "WARNING",
      "SUGGESTION",

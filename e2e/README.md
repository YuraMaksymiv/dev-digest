...
| Spec | Flow |
|------|------|
| `01-app-boot` | root → redirect to first repo's PR list → heading + FINDINGS column |
| `02-repo-pulls-detail` | PR list → open PR #482 → review detail route |
| `03-agents` | agents list renders the seeded reviewer agents |
| `04-pr-findings` | PR #482 → Agent runs tab → seeded run verdict + findings; expand → FindingCard; severity counters filter the list |
...
| `08-skills` | `/skills` → seeded skill in the rail; agent → Skills tab → linked-count chip + a skill row |
